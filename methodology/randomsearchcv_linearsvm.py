# Randomized Search Cross Validation for LinearSVM:
import pandas as pd
import numpy as np
import time
import os
...
grid_param =   {'tfidf__ngram_range': tfidf__ngram_range,
                'svm__average': svm__average, 
                'svm__l1_ratio' : svm__l1_ratio, 
                'svm__alpha': svm__alpha
                }
...
# fit model
if os.path.isfile('randomsearchcv_linearsvm.pkl'):
    print('Step 4: loading model...')
    best_estimator = joblib.load('randomsearchcv_linearsvm.pkl')
else:    
    print('Step 4: fitting randomized search grid...')
    t0 = time.time()
    search = RandomizedSearchCV(pipeline, param_distributions=grid_param, verbose=5, cv=5, n_iter=100, random_state=42)
    ...
    joblib.dump(best_estimator, 'randomsearchcv_linearsvm.pkl')
